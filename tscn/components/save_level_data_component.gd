	...
	if nodes!=null:
		for node in nodes:
			if node is SaveDataComponent:
				var save_data_resouce:NodeDataResource = node.save_data()
				var save_final_resouce = save_data_resouce.duplicate()
				game_data_resource.save_data_nodes.append(save_final_resouce)
