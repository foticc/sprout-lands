...
var tomato_scence = preload("res://tscn/objects/collectable/tomato_fruit.tscn")
...
func _on_crop_harvesting()->void:
	var tomato_instance = tomato_scence.instantiate() as Node2D
	tomato_instance.global_position = global_position
	get_parent().add_child(tomato_instance)
	queue_free()
